...
static void vfio_init_fault_regions(VFIOPCIDevice *vdev, Error **errp)
{
    ...
    ret = vfio_get_dev_region_info(&vdev->vbasedev,
                                   ...
                                   &fault_region_info);
    if (ret) {
        error_setg_errno(errp, -ret, "failed to get fault prod region info");
        return;
    }
    ...
    cap_fault = container_of(hdr, struct vfio_region_info_cap_fault, header);
    max_version = cap_fault->version;

    fault_region_name = g_strdup_printf("%s FAULT PROD %d", vbasedev->name,
                                        fault_region_info->index);

    ret = vfio_region_setup(OBJECT(vdev), vbasedev,
                            &vdev->fault_prod_region, fault_region_info->index,
                            fault_region_name);
    if (ret) {
        error_setg_errno(errp, -ret, "failed to setup the fault prod region %d",
                         fault_region_info->index);
        goto out;
    }
    ...
    ret = vfio_get_dev_region_info(&vdev->vbasedev,
                                   ...
                                   &fault_region_info);
    if (ret) {
        error_setg_errno(errp, -ret, "failed to get fault cons region info");
        return;
    }

    fault_region_name = g_strdup_printf("%s FAULT CONS %d", vbasedev->name,
                                        fault_region_info->index);

    ret = vfio_region_setup(OBJECT(vdev), vbasedev,
                            &vdev->fault_cons_region, fault_region_info->index,
                            fault_region_name);
    if (ret) {
        error_setg_errno(errp, -ret, "failed to setup the fault cons region %d",
                         fault_region_info->index);
    }
    ...
    if (bytes != 4) {
        error_setg(errp, "Unable to set the chosen fault ABI version (%d)",
                   max_version);
    }
out:
    g_free(fault_region_name);
...
static void vfio_dma_fault_notifier_handler(void *opaque)
{
    ...
    if (!queue) {
        ...
        error_report("%s: fault queue is not mmapped: fault handling will be slower",
                     vdev->vbasedev.name);
    ...
    while (vdev->fault_cons_index != header.prod) {
        memory_region_inject_faults(iommu_mr, 1, &queue[vdev->fault_cons_index]);
...
static void vfio_exitfn(PCIDevice *pdev)
{
    ...
    vfio_set_event_handler(vdev, VFIO_PCI_DMA_FAULT_IRQ_INDEX,
                           false, NULL, &err);
    if (err) {
        warn_reportf_err(err, VFIO_MSG_PREFIX, vdev->vbasedev.name);
    }
