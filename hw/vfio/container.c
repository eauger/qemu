...
static int legacy_attach_device(VFIODevice *vbasedev, AddressSpace *as, Error **errp)
{
    ...
    container = vfio_connect_container(group, as, &legacy_ops, errp);
    if (!container) {
    ...
    if (ret) {
        vbasedev->iommu_ops->vfio_iommu_detach_device(vbasedev, as, errp);
	return -1;
    }
    return 0;
}

static int legacy_detach_device(VFIODevice *vbasedev, AddressSpace *as, Error **errp)
{
    int groupid = vfio_device_groupid(vbasedev, errp);
    VFIOGroup *group;
