...
static void qpci_arm_config_writeb(QPCIBus *bus, int devfn, uint8_t offset, uint8_t value)
{
    uint64_t addr = bus->ecam_alloc_ptr + ((0 << 20) | (devfn << 12) | offset);
    uint32_t val = value;
...
static void qpci_arm_config_writew(QPCIBus *bus, int devfn, uint8_t offset, uint16_t value)
{
    uint64_t addr = bus->ecam_alloc_ptr + ((0 << 20) | (devfn << 12) | offset);
    uint32_t val = value;
...
static void qpci_arm_config_writel(QPCIBus *bus, int devfn, uint8_t offset, uint32_t value)
{
    uint64_t addr = bus->ecam_alloc_ptr + ((0 << 20) | (devfn << 12) | offset);
    uint32_t val = value;
...
void qpci_init_arm(QPCIBusARM *qpci, QTestState *qts, QGuestAllocator *alloc, bool hotpluggable)
{
    assert(qts);
    ...
    qpci->bus.qts = qts;
    qpci->bus.pio_alloc_ptr = 0x3eff0000;
    qpci->bus.pio_limit = 0x3F000000;
...
QPCIBus *qpci_new_arm(QTestState *qts, QGuestAllocator *alloc, bool hotpluggable)
{
